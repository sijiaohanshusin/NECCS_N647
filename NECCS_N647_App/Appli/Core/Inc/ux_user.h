...
/* NECCS: the storage class thread runs OUR media callbacks (sd_nand HAL
 * polling I/O) on this stack; give it headroom over the 1 KB port
 * default. The stack comes out of the UX pool, which MUST stay in
 * internal RAM (HyperRAM-backed thread stacks corrupted the restored
 * context - board 2026-07-20: INVSTATE HardFault, PC pointing into the
 * pool, registers full of the 0xEF stack-fill pattern). */
...
/* NECCS: default 20 never runs on this system - the acoustic thread is a
 * by-design priority-13 CPU hog, so USBX threads must sit at the media
 * level. (Throughput note, board 2026-07-20: FS-mode MSC moves ~9 KB/s
 * with the USB IRQ rate pinned near the 100 Hz tick; priority 9 vs 12
 * changed nothing, so the pacing is not thread starvation - suspected
 * per-transfer turnaround in the FS chirp-less path. Revisit together
 * with the HS upgrade once OTG1 gets a clean data path.) */
