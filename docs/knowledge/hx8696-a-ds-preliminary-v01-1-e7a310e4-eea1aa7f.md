...
Preliminary version 01 August, 2010

<!-- page: 2 -->
HX8696-A
1200CH TFT LCD Gate Driver

PreliminaryVersion01 August, 2010

1. General Description

The HX8696-A is a 1200-channel outputs gate driver, which is used for driving the
...
z 2-level output
z 1200/1152/1080/960 channel outputs selectable with 2 dummy outputs
z Various scan function can support various dual gate driving sequence
z Maximum 200KHz operation frequency
z Digital supply voltage:2.2V to 3.6V
z LCD driving voltage range: 40V
z Bi-directional data shift capability
z High voltage CMOS process technology
z COG/COF package
...
in whole or in part without prior written permission of Himax. August, 2010
...
OUT0 OUT1 OUT2 OUT3
(1) OUT1198 OUT1199 OUT1200 OUT1201
(1)
...
CPV STV1 OE1 OE3 MODE2 SEG2
L/R STV2 OE2 MODE1 SEG1 EVEN
...
The shift direction of device internal shift register is
Shift direction controlled by this pin as shown below: Default L/R=L.
L/R In
control pin L/R =H: STV1ÆOUT1ÆOUT2Æ - - - ÆOUT1200ÆSTV2.
L/R =L: STV2ÆOUT1200Æ - - - ÆOUT2ÆOUT1ÆSTV1.
L/R=H, STV1 is used for start pulse input;
STV1 Start pulse STV2 is used for start pulse output.
I/O
STV2 input/output pin L/R =L, STV2 is used for start pulse input;
STV1 is used for start pulse output.
The OE signal controls the output enable.
OE1 OE1~OE3=H: All driver outputs are fixed to VEE
Output enable
OE2 In regardless of CPV. However, the content of
control
OE3 shift register is not cleared.
OE1~OE3=L: Normal operation.
When /XAO is set to L, all outputs are fixed to VGH. Note
that this pin has higher priority than OE. However, the Output all-on
/XAO In content of shift register is not cleared.
control /XAO is pulled high to VDD internally. When it is not used,
connecting to VDD is recommended.
Channel mode selection, default MODE1=H, MODE2=H.
Output Disable
MODE1 MODE2
channel channel
MODE1 Channel mode 960 481~720 L L In
MODE2 selection 1080 541~660 H L
1152 577~624 L H
1200 - H H
Scan function selection, default SEG1=L, SEG2=L.
Scan function SEG1 SEG2
SEG1 Scan function Z1 L L
In SEG2 selection Z1 L H
弓 H L
Z2 H H
Frame selection, default EVEN=L.
Frame EVEN
EVEN In Frame selection Odd L
Even H
OUT1~ The output is either VGH or VGL for driving the gate line of
Out Driver output
OUT1200 TFT LCD panel.
OUT0
Out Auxiliary pins These two pins always output VGL level.
OUT1201
...
VGL In Power supply Power supply for Gate off output.
...
When L/R=H and MODE=H, the STV1 start pulse input is sensed on the rising edge
of CPV and stored in the first stage of shift register, which makes the first scan signal
output from the OUT1 pin. While stored data is transferred to the next stage shift
register on the rising edge of next CPV, new data of STV1 is sensed and stored
simultaneously.
The output pin (OUT1 to OUT1200) supplies VGH voltage or VGL voltage to the LCD
panel depending on the data stored in the shift register. For normal operation, a VGH
voltage is outputted one by one from OUT1 to OUT1200 in synchronization with CPV
pulse. According to SEG1, SEG2, and EVEN pins setting, it is possible to get other
different output sequence (Described in section 5.2).
After 1200 CPV rising edge are past, the STV2 goes up to high level at the 1200th
falling edge of CPV and goes down to low level at the 601st falling edge of CPV. This
STV2 output signal becomes the STV1 start pulse input of next cascaded gate driver.
When OE=H, the corresponding output channels are fixed to VGL level regardless of
CPV. The channel output returns to normal status as soon as OE goes back to L.
...
1 2 3 4 5 6 7 8 1198 1199 1200 1201 1202
CPV
...
Example of input/output timing
(MODE2=H, MODE1=H, SEG1=L, SEG2=L, EVEN=H, L/R=H)
...
The logic levels of CPV, L/R, OE1, OE2, OE3, /XAO, MODE1, MODE2, SEG1, SEG2,
EVEN, STV1 and STV2 have to swing between VDD for “H” and VSS for “L”. PATH2
has ESD protection diodes connected to VDD and to VSS, the signal on PATH2
should also swing between VDD and VSS.

5.3 Power on/off sequence

HX8696-A has a latch up free design. However, to prevent possible power on display
noise or possible power off image residue, the power on/off sequence shown below
need to be followed.

When power on, VGH/VGL can start to be turned on after VDD reaches 1.8V. After
VGH/VGL start to be turned on, CPV and STV should be not floating, and /XAO should
be at VDD level or floating. The other control signals have no timing limitation. If the
possible power on display noise is not concerned or the backlight is delayed to turn on
to shadow the possible noise, there is no power on sequence limitation.

When power off, VGH/VGL must start to be turned off before VDD drops to 1.8V. If the
possible power off image residue is not concerned or the backlight is turned off in
advance to shadow the possible image residue, there is no power off sequence
limitation.

When power on: VDD→VGH/VGL
...
Min. Typ. Max.
Power supply voltage 1 VGH -0.3 - +42.0 V
Power supply voltage 2 VDD -0.3 - +7.0 V
Power supply voltage 3 VGL VGH-42 - +0.3 V
Input voltage VIN -0.3 - VDD+0.3 V
Storage temperature TSTG -55 - +125 ℃
Note: Device will probably be damaged permanently in case that the stresses are over the absolute maximum
...
Power supply voltage 4 VGH -VGL 12 - 40 V
Operation frequency FCPV - - 200 KHz
Operation temperature TA -40 - +95 ℃

6.3 Electrical characteristics (VSS=0V)

Applicable Spec. Parameter Symbol Condition Unit
pin Min. Typ. Max.
Input H voltage VIH All input pins - 0.7VDD - VDD
Input L voltage VIL All input pins - VSS - 0.3VDD V
Output H voltage VOH STV1,2 IOH=40µA VDD-0.4 - VDD
Output L voltage VOL STV1,2 IOL=40µA VSS - VSS+0.4
OUT1 ~ VOUT= Output H resistance ROH - - 1000 Ω OUT1200 VGH-0.5V
OUT1 ~ VOUT= Output L resistance ROL - - 1000 Ω OUT1200 VGL+0.5V
Input leakage current IIN Note(1) - -1.0 - +1.0 µA
VDD=3.3V,
Pull high/low resistance RPU Note(2) 70 200 400 kΩ TA=25℃
Pull high resistance RPU /XAO VIN=VSS 40 - 200 kΩ
VGH Power
IVGH VGH - - 200 consumption Note(3) µA
VDD Power
IVDD VDD - - 100 consumption
...
Spec. Parameter Symbol Condition Unit
Min. Typ. Max.
CPV period tCPV - 5 - -
CPV pulse width tCPVH, tCPVL 50% duty cycle 2.5 - -
OE pulse width tWOE - 1 - -
/XAO pulse width tWXAO - 10 - -
Data setup time tSU - 1.0 - -
Data hold time tHD - 1.0 - - µs
EVEN setup time tMSU - 1.0 - -
EVEN hold time tMHD - 1.0 - -
CPV to output delay time tPD1 CL=300pF - - 1.2
Start pulse output delay time tPD2 Loading=30pF - - 1.2
OE to output delay time tPD3 CL=300pF - - 0.8
/XAO to output delay time tPD4 CL=300pF - - 50
...
6 5 4 3 2 1 1485 1483 285 283 282 281 280 279 278 277 276
OE1 7 8 275 274 OE1
OE2 9 10 273 272 OE2
OE3 11 12 1484 1482 286 284 271 270 OE3
L/R 13 14 Y 269 268 L/R
CPV 15 16 267 266 CPV
PATH2 PATH2 X 265 264

263 262 STV1 STV2 1719 1820 (0,0)
VGH 21 22 HX8696-A 261 260 VGH
VGL 23 24 259 258 VGL
VDD VDD
VSS VSS
25 26 27 28 29 30 31 32 33 249 250 251 252 253 254 255 256 257

View from bump side

Chip size: 23650µm x 670µm (scribe line included)
Scribe line: 80µm
Bump height: 12±3µm
Total area of IC bump: 3214120µm2
...
403 403
89 115 199 36 36 36 36 199 115 89
...
2010/03/10 Page 12
Add PATH2 description in section 5.2.
Page 16
Correct the title of section 9.1.
2010/08/27 Page 12
Modify section 5.3: Power on/off sequence.
