...
## 五、云台转接板(USB3-A 口)引脚定义(2026-07-19 审定, 画板依据)

电源板为 USB3.0-A 母座(HC-USB3.0-C34-P), 转接板用 A 公头对插:

| USB3-A 脚 | 信号 | N6 侧 | 转接板用途 |
|---|---|---|---|
| 1 (VBUS 位) | **7.4V 舵机轨**(过 F3) | PC7=GIMBAL_PWR_EN 开轨 | 舵机 VCC ×2 |
| 2 (D−) | NC(板上未接) | - | 悬空 |
| 3 (D+) | NC | - | 悬空 |
| 4 (GND) | GND | - | 主回流 |
| 5 (SSRX−) | GPIO1 = PAN PWM | **PE13** TIM1_CH3 | 水平舵机信号 |
| 6 (SSRX+) | GPIO2 = TILT PWM | **PE14** TIM1_CH4 | 俯仰舵机信号 |
| 7 (GND_DRAIN) | NC | - | 悬空 |
| 8 (SSTX−) | GPIO3 = 备用 | PA12(输入, 已释放) | 测试点 SPARE |
| 9 (SSTX+) | **3.3V 激光轨**(U6 输出) | PC10=LASER_EN 开轨 | 激光 VCC |
| 10-13 (壳) | GND | - | 全焊, 补充回流 |

PWM: 50Hz / 500-2500µs / 3.3V 电平 / 中位 1500µs / 800µs·s⁻¹ 限幅。
GPIO4(继电器, PD0)只在 U8 GH-8 线束上, USB3-A 转接板没有继电器线。

转接板设计要点:
1. 两条 PWM 线各加 100k 下拉(MCU 复位期防杂脉冲), 可选串 220Ω。
2. 舵机端 ≥470µF/16V 低 ESR + 100nF(双 MG 舵机浪涌大, USB 单 pin 载流有限)。
3. **查电源板 F3**(0603, SMD0603-010-15, 整条 7.4V 都过它): 双舵机堵转
   必超, 预计换 ≥3A 或 0Ω; 顺带查 F2(整机 5V 同款)。
4. 丝印"非 USB! 7.4V": 误用 A-A 线插电脑会损伤主机。
5. 激光 VCC=pin9: PC10 可控的前提是 §4.1 改动(剪 FPC18/19、拆 R14、
   U6 分压 16.5k)已完成; 未改前 pin9 为常电 3.3V。
6. pin8 引出焊盘即可, 默认不接。

## 六、排障记录更正(2026-07-19)

2026-07-18 夜 pcmd I2C 探测时好时坏(present 0x1/0x3/0x7 跳变), 曾先后怀疑
release 残留(全片擦除后依旧)、固件改动(回退到基线 b702c1b1 同样复现)、
麦阵排线松动。实测定位: **供电问题** —— 大屏(7 寸新总成)负载下轨压不足,
换小屏即恢复; 供电修复后大小屏均正常, pcmd 恢复全绿。此前提交信息里
"疑似麦阵 FPC"的猜测作废, 以本条为准。
