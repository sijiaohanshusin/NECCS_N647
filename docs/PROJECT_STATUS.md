...
| 定位校准方法论(重要) | 两个根因闭环:1) **连续正弦单音在实验室(玻璃窗强反射)形成驻波场**——tone bin 幅度跨麦 24 倍散布、SRP 候选全部钉死在搜索边缘(仰角±60-62°伪峰)、q=0;换**宽带噪声**立即 conf=100 稳定锁定(左/右声道各自独立方位,复现可靠)。校准/验收一律用噪声或真实宽带源,别用单音。2) 相机 FOV 扩大(2560x1920 裁剪)后 overlay 常数仍是 77x61.1°→所有准心/热图缩放错一倍;按裁剪光学重算为 50.4x38.9°(app_acoustic_service.h + Model.cpp 两处)。附:接线乱序假设已排除——**PCMD3180 就焊在麦阵板上**(阵列网表:U1/U2→SDOUT1、U3/U4→SDOUT2,DIN 经串阻直连各麦),FPC 只传 TDM+I2C,重插不可能打乱麦序;I2C present=0xf 也证明 FPC 无错位。原始帧相位拟合脚本 _debug_logs/phase_fit.py 可复用 | 3a585d30 |
| 相机-声学外参标定(耳机实测定稿) | 白纸背景+单只耳机放宽带噪声、逐点"像素↔角度"对账发现:**镜头是广角款而非树莓派 62.2° 规格**——两个独立位置的方位角都精确落在 tan 域 1.77 倍缩放上(反推全幅 HFOV≈94°,旧代码的 77° 是厂商标称、比 62.2° 更接近真相)。定稿常数(2560x1920 裁剪):**HFOV=80.0° / VFOV=64.4°**(方形像素几何 tanV=0.75·tanH),无偏移项。板验:方位角 3 个干净点误差 ≤2.4°(P1 精确 0.0°、P2 0.6°、挡反射后的纸箱点 2°);俯仰最干净点(纸中央 q=13)误差 0.6°,其余轮次 ±10-20° 残差全部对应可见的垂直多径(纸面上缘/桌面/水瓶/玻璃窗——挡板实验证实),均值≈0 → 无系统性安装俯仰角。**声学教训:室内标定俯仰必被水平反射面污染,信最干净(高 q)的点;双耳机会同时发声(增益器串扰),标定必须单只**。工具:play_tone_ch.ps1(noise 模式)、track_sample.gdb、离线 gcc_fit.py(宽带互相关拟合,比追踪器抗噪)。彩蛋:宽带噪声被 NPU 声纹分类为"气体泄漏 94%",分类器工作正常 | 本次 |
| PCMD 自愈看门狗(掉线+位偏移) | 现场实证 2026-07-23 两种运行时故障形态,I2C 回读都全绿、帧流不断,cfg_ok 和帧停看门狗都不报:1) **哑片**——芯片一颗颗掉出 TDM 总线(先 U2/U4,后 U3),整片槽位读 -90 地板,int_latch0 有时钟错误锁存,与 FPC 重插后时钟边际吻合(2026-07-09 历史:每总线第二片本就时序敏感);2) **TDM 位偏移**——重建可能把某片的输出对齐掷坏 ~8 BCLK:采样值=真值<<8(低 8-9 位恒零 0x0600/0x1200/0x0401),静息本底放大 256 倍读 -20 dBFS(用户上报的"静息 -20 多的 MIC"),该片首槽落进帧间空隙恒 -90,邻片交界槽被重叠污染。实验证明 DMA-only 重启治不了,**同模式完整重建必治但属于掷骰子**(能治哑片也可能掷出位偏移)→ 策略=重建后检测+不合格再掷。落地:哑检测(芯片粒度,整片 -90 持续 ~3s;单麦死不触发)+ 位偏移检测(**声音无关签名**:热槽 raw≥128 且窗口低位 OR 的 bit1-3 恒零——真实声音永远抖动低位,伪造不出;128 次 flush/窗 ×3 窗 streak)→ 同模式 ExecuteModeSwitch 全重建,预算 3 次、持续健康 10s 回满,快速扑动自动耗尽停手。计数器跨重建保留(顺带修了 wdog 计数被 ClearRuntime 清零的旧问题)。快照 silent_chip_mask/shift_chip_mask/silent_restart_count,status 透出 silent/shift/heals。板验:6min 浸泡零误报,手动触发路径治愈两种故障各一次。根因是硬件(FPC 完整性),固件自愈是缓解;换正式 FPC 后回归。零散单麦哑/复活(A8/A9、B0/B1 呼吸,B12 现死)是阵列板触点级,固件不处理 | 本次 |
